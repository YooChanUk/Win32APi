...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, //실행될 프로세스의 시작 주소
                     ...
                     _In_ int       nCmdShow)
{
    ...
    //core초기화
    if (FAILED(CCore::GetInst()->init(g_hWnd, POINT{500,500})))
    {
    ...
    //msg.message == WM_QUIT인 경우 false를 반환 -> 종료

    //PeekMesssage
    //메세지 유무와 관계없이 반환
    //메세지 큐에서 메세지를 확인한경우 true 메세지 큐에 메세지가 없는경우 false반환
